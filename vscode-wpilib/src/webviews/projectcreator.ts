...
export class ProjectCreator extends WebViewBase {
  ...
  private constructor(
    ...
    super(
      ...
      i18n('projectcreator', 'WPILib Project Creator'),
      resourceRoot
    );
  ...
  private async createProject(data: IProjectIPCData) {
    if (!path.isAbsolute(data.toFolder)) {
      vscode.window.showErrorMessage(
        i18n('message', 'Can only extract to absolute path')
      );
      return;
    }
    ...
    const toFolder = data.newFolder
      ? path.join(data.toFolder, data.projectName)
      : data.toFolder;
  ...
  private async handleProjectType() {
    const items = [];
    items.push({
      label: i18n('projectcreator', 'Template'),
      value: ProjectType.Template,
    });
    items.push({
      label: i18n('projectcreator', 'Example'),
      value: ProjectType.Example,
    });
  ...
  private async handleBase(data: IProjectIPCData) {
    const result = await vscode.window.showQuickPick(
      this.exampleTemplateApi.getBases(
        data.projectType === ProjectType.Template,
        data.language
      ),
      {
  ...
  private async handleNewProjectLoc(data: IProjectIPCData) {
    const open: vscode.OpenDialogOptions = {
      ...
      defaultUri:
        data.toFolder.length > 0 ? Uri.file(data.toFolder) : undefined,
    };
