...
async function globalProjectSettingUpdate(
  message: string
): Promise<IUpdatePair | undefined> {
...
export function createVsCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('wpilibcore.startRioLog', async () => {
      ...
      await externalApi
        .getDeployDebugAPI()
        .startRioLog(await preferences.getTeamNumber(), true);
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (source: vscode.Uri | undefined) => {
        ...
        await externalApi
          .getDeployDebugAPI()
          .simulateCode(workspace, source, '-PhwSim');
      }
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('wpilibcore.setLanguage', async () => {
      ...
      if (deployDebugApi.getLanguageChoices().length <= 0) {
        vscode.window.showInformationMessage(
          i18n('message', 'No languages available to set')
        );
        return;
      }
      ...
      const result = await vscode.window.showQuickPick(
        deployDebugApi.getLanguageChoices(),
        {
          placeHolder: i18n(
            'ui',
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'wpilibcore.setSkipSelectSimulateExtension',
      async () => {
        const preferencesApi = externalApi.getPreferencesAPI();
        const workspace = await preferencesApi.getFirstOrSelectedWorkspace();
        if (workspace === undefined) {
        ...
        if (result === undefined) {
          logger.log(
            'Invalid selection for skipping of selection of simulation extensions'
          );
          return;
        }

        await preferences.setSkipSelectSimulateExtension(
          result.yes,
          result.global
        );
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'wpilibcore.setSelectDefaultSimulateExtension',
      async () => {
        const preferencesApi = externalApi.getPreferencesAPI();
        const workspace = await preferencesApi.getFirstOrSelectedWorkspace();
        if (workspace === undefined) {
        ...
        if (result === undefined) {
          logger.log(
            'Invalid selection for selection of all simulation extensions by default'
          );
          return;
        }

        await preferences.setSelectDefaultSimulateExtension(
          result.yes,
          result.global
        );
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'wpilibcore.setStopSimulationOnEntry',
      async () => {
        const preferencesApi = externalApi.getPreferencesAPI();
        const workspace = await preferencesApi.getFirstOrSelectedWorkspace();
        if (workspace === undefined) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('wpilibcore.setUseWinDbgX', async () => {
      ...
      const wpConfiguration = vscode.workspace.getConfiguration(
        'wpilib',
        workspace.uri
      );
      ...
      const result = await globalProjectSettingUpdate(
        i18n(
          'message',
          'Use WinDbg Preview (from store) for windows debugging? Currently {0}',
          res
        )
      );
      if (result === undefined) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('wpilibcore.changeDesktop', async () => {
      ...
      const result = await vscode.window.showInformationMessage(
        i18n(
          'message',
          'Enable Desktop Support for Project? Currently {0}',
          isEnabled
        ),
        { modal: true },
        i18n('ui', 'Yes'),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'wpilibcore.openApiDocumentation',
      async () => {
